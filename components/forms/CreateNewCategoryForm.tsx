...
export default function CreateNewCategoryForm() {
  ...
  return (
    ...
    >
      ...
      <h1 className="text-xl font-semibold mb-4">Category Image</h1>
        {imageUrl ? (
          <div className="rounded-lg overflow-hidden relative">
            <Image src={imageUrl} width={500} height={500} className="" alt="category image" />
            <button onClick={()=>setImageUrl("")} className="bg-white text-xs rounded py-1 px-2 font-semibold transition duration-50 absolute top-2 right-2 text-black hover:opacity-65" type="button">Remove</button>
          </div>
        ) : (<UploadDropzone
        className="dropzone"
      ...
      />)}
        <div className="w-full flex"><Button label="Create Category" solid className="border rounded bg-secondary border-none px-3 py-2 mt-4 ml-auto"/></div>
      </div>
    </form>
  )
}
